...
VLLM_HOST_MODEL_NAME='llama3_70b'
VLLM_PORT_NUMBER=8050
# Generation MODE
# NOTE available modes: "default", "detailed", "concise", "quality_enhancement", "copy_paste"
# Max workers
MAX_WORKERS=20
# Output folder
OUTPUT_FOLDER='generated_data_llama3_70b_with_length'
...
for dataset_name in 'alpaca' 'koala' 'vicuna' 'sinstruct' 'wizardlm' 'lima'; do
    for MODE in 'default' 'biased' 'toxic' 'inconsistent' 'poor_reasoning' 'verbose' 'final_relevance' 'final_logical'; do
        INPUT_JSON=./reference/generation_template/${dataset_name}_template.json
        python ./generation/generate_different_dataset_vllm_with_length.py \
